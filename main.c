...
//    console_print("response time: %u\n", elapsed);
    if (response_time > 0) {
        console_print("response time: %u (expected %u)\n", elapsed, response_time);
    }
...
static void send_add() {
    uint8_t add[] = {
            ...
            (uint8_t) xTaskGetTickCount() >> 25,
            (uint8_t) xTaskGetTickCount() >> 4,
            (uint8_t) xTaskGetTickCount() >> 19,
    };
...
static void send_delay() {
    uint8_t delay[] = { 0x43, 0x02, (uint8_t) xTaskGetTickCount(), (uint8_t) xTaskGetTickCount() >> 3 };
    uint8_t result[] = { 0x51, delay[3] };
    execute(delay, sizeof(delay), result, sizeof(result), 0x200 + delay[2]);
}
